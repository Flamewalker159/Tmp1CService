...
namespace Tmp1CService.Security;
    ...
    IConfiguration configuration) : AuthenticationHandler<AuthenticationSchemeOptions>(options, logger, encoder)
{
    ...
    {
        var authHeader = Request.Headers["X-API-KEY"].FirstOrDefault();
        var staticKey = configuration["ApiKey:Key"];

        if (string.IsNullOrWhiteSpace(authHeader) || authHeader != staticKey)
